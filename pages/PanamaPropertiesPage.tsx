...
const PanamaPropertiesPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const panamaProperties = data.properties.filter(
          p => p.country_label && p.country_label.toLowerCase().trim() === 'panamá'
        );

        setAllProperties(panamaProperties);
        
        const panamaCities = [...new Set(panamaProperties.map(p => p.city_label).filter(Boolean))];
        setCities(['all', ...panamaCities.sort()]);
    ...
    <div className="bg-brand-white">
      <div className="container mx-auto px-6 pt-36 pb-24">
        <header ref={pageHeaderRef} className="text-center mb-16">
          <h1 className="text-5xl md:text-6xl font-bold text-brand-black">Propiedades en Panamá</h1>
          <p className="text-lg text-gray-600 mt-4 max-w-2xl mx-auto">Descubre oportunidades únicas en el corazón de las Américas.</p>
        </header>
...
export default PanamaPropertiesPage;
